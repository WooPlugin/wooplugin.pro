...
const GITHUB_REPO = 'WooPlugin/product-feed-for-woocommerce-pro';
...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    if (licenseKey) {
      ...
      // Validate download token (from /api/license/validate)
    ...
    // License is valid - redirect to GitHub release asset
    // For private repos, you'd fetch with a GitHub token and proxy the download
    ...
    const zipAsset = releaseData.assets?.find((asset: any) =>
      asset.name.endsWith('.zip') && asset.name.includes('product-feed')
    );
    ...
    if (!zipAsset) {
      ...
      // For private repos, we need to proxy the download
      if (githubToken) {
        ...
        return new Response(zipResponse.body, {
          ...
          headers: {
            ...
            'Content-Disposition': `attachment; filename="product-feed-woocommerce-pro-${CURRENT_VERSION}.zip"`,
          },
        });
      }

      // For public repos, redirect directly
...
/**
 * Validate a time-limited download token
 */
function validateDownloadToken(token: string, licenseKey: string): boolean {
  try {
    ...
    // Verify hash (simplified - in production use proper crypto)
